...
async function runTests() {
  ...
  await Promise.all([
    new Promise((r) =>
      clientA.once('connect', () => {
        assert(true, 'Client A connected');
        r();
      }),
    ),
    new Promise((r) =>
      clientB.once('connect', () => {
        assert(true, 'Client B connected');
        r();
      }),
    ),
    new Promise((r) =>
      clientC.once('connect', () => {
        assert(true, 'Client C connected');
        r();
      }),
    ),
  ]);
  ...
  const receiveBPromise = new Promise((resolve, reject) => {
    const timeout = setTimeout(
      () => reject(new Error('receive_message not received by B within 2 s')),
      2000,
    );
  ...
  const receiveCPromise = new Promise((resolve, reject) => {
    const timeout = setTimeout(
      () => reject(new Error('receive_message not received by C within 2 s')),
      2000,
    );
  ...
  if (resultB._socketError) {
    ...
    assert(
      true,
      'Server responded gracefully (DB unavailable): ' + resultB._socketError,
    );
  } else {
    // DB connected — the full message was persisted and broadcast
    assert(
      resultB.message === 'Hello from the test script to group!',
      'Client B received message text matches',
    );
    assert(
      resultC.message === 'Hello from the test script to group!',
      'Client C received message text matches',
    );
    assert(!!resultB._id, 'Message has a MongoDB _id');
    assert(!!resultB.createdAt, 'Message has a createdAt timestamp');
    assert(
      Array.isArray(resultB.receiverIds) && resultB.receiverIds.length === 2,
      'Message has 2 receiverIds',
    );
  }
  ...
  const echoPromise = new Promise((resolve, reject) => {
    const timeout = setTimeout(
      () => reject(new Error('echo not received')),
      2000,
    );
    clientA.once('receive_message', (msg) => {
      clearTimeout(timeout);
      resolve(msg);
    });
    clientA.once('error', (err) => {
  ...
  if (echo._socketError) {
    ...
    assert(
      ...
      'Sender also receives echo of own message',
    );
  }
