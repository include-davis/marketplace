    ...
    socket.on(
      ...
      async (data: {
        ...
        if (!isValidObjectId(conversationId)) {
          socket.emit('error', {
            message: `Invalid conversationId: must be 24-character hex string, got "${conversationId}" (${conversationId.length} chars)`,
          });
          return;
        }
        if (!isValidObjectId(senderId)) {
          socket.emit('error', {
            message: `Invalid senderId: must be 24-character hex string, got "${senderId}" (${senderId.length} chars)`,
          });
          return;
        }
